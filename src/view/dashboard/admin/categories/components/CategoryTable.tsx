...
export function CategoryTable({
  ...
  const renderItemsWithTooltip = (items: string[], type: 'sub' | 'skill', cat: Category) => {
    const maxVisible = 2;
    const visibleItems = items.slice(0, maxVisible);
    const remainingItems = items.slice(maxVisible);
    const onAdd = type === 'sub' ? () => onAddSubcategory(cat) : () => onAddSkill(cat);
              ...
              <TooltipContent className="bg-popover text-popover-foreground border-border z-50 max-w-[280px] rounded-xl border p-2.5 shadow-xl">
                <div className="space-y-1.5">
                  <p className="text-muted-foreground/85 text-[9px] font-bold tracking-wider uppercase">
                    {type === 'sub' ? 'Sub-categories' : 'Skills'} ({items.length})
                  </p>
            ...
            {isLoading
              ...
              : categories.map((cat) => (
                  ...
                  >
                    ...
                    <TableCell>{renderItemsWithTooltip(cat.subcategories, 'sub', cat)}</TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-bold">{cat.totalJobs}</span>
                      ...
                      <div className="flex items-center justify-end gap-3">
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button variant="ghost" className="hover:bg-muted h-8 w-8 rounded-lg">
                          ...
                          >
                            ...
                            <DropdownMenuItem
                              ...
                              onClick={() => handleToggleStatus(cat.id, cat.name)}
                            >
                              {cat.active ? (
                                <>
      ...
      >
        <DialogContent className="bg-card rounded-2xl sm:max-w-[450px]">
          <DialogHeader>
            ...
            <DialogDescription>
              All registered {viewDialog.type === 'sub' ? 'sub-categories' : 'technical skills'}{' '}
              under this category.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
        ...
        <AlertDialogContent className="bg-card max-w-[420px] rounded-2xl border">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-muted-foreground/80 pt-2 text-sm leading-relaxed font-semibold">
              ...
              ? This will permanently remove the category, its subcategories, and all associated
              technical skills.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="mt-4 gap-2">
            ...
            <AlertDialogAction
              onClick={async () => {
                if (categoryToDelete) {
                  await handleDelete(categoryToDelete.id, categoryToDelete.name);

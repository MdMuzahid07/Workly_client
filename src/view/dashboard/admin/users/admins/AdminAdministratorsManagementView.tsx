...
const unwrapApiData = <T,>(payload: ApiEnvelope<T> | T | undefined): T | undefined => {
...
const downloadCsv = (filename: string, rows: Record<string, string>[]): void => {
...
const AdminAdministratorsManagementView = () => {
  ...
  const [auditStaffFilter, setAuditStaffFilter] = useState<AdminStaffRow | null>(null);
  ...
  const [permissionsTarget, setPermissionsTarget] = useState<AdminStaffRow | null>(null);
  ...
  const [setStaffStatus, { isLoading: isUpdatingStatus }] = useSetStaffStatusMutation();
  ...
  const staffMeta = ((staffData as ApiEnvelope<AdminStaffRow[]> | undefined)?.meta ?? null) as {
    page?: number;
    totalPage?: number;
  } | null;
  ...
  const auditMeta = ((auditData as ApiEnvelope<AdminAuditLogRow[]> | undefined)?.meta ?? null) as {
    page?: number;
    totalPage?: number;
    total?: number;
  } | null;
  ...
  const handleConfirmStatusChange = async () => {
    ...
    try {
      ...
      toast.success(`Administrator ${isActive ? 'activated' : 'deactivated'} successfully`);
      setStatusTarget(null);
    } catch (error: unknown) {
      toast.error(
        (error as { data?: { message?: string } })?.data?.message || 'Failed to update status',
      );
    }
  };
  ...
  const handleSaveStaffRole = async (userId: string, role: AdminStaffRole) => {
    try {
      ...
      toast.error(
        (error as { data?: { message?: string } })?.data?.message || 'Failed to update staff role',
      );
      throw error;
    }
  };
  ...
  const handleExportLogs = async () => {
    ...
    try {
      ...
      do {
        ...
        const meta = (result as ApiEnvelope<AdminAuditLogRow[]> | undefined)?.meta as
      ...
      downloadCsv(`workly-audit-logs-${format(new Date(), 'yyyy-MM-dd')}.csv`, rows);
      toast.success(`Exported ${rows.length} audit log${rows.length === 1 ? '' : 's'}`);
    } catch (error: unknown) {
      const message =
        (error as { data?: { message?: string } })?.data?.message || 'Failed to export audit logs';
      toast.error(message);
    } finally {
      ...
      <div className="space-y-8 px-4 py-6 sm:px-6 sm:py-8">
        ...
        {(isStatsError || isStaffError) && (
          <div className="border-destructive/30 bg-destructive/5 flex flex-col gap-3 rounded-xl border p-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Button variant="outline" className="rounded-full font-bold" onClick={handleRetry}>
              Retry
            </Button>
          </div>
        )}
          ...
          <Card className="overflow-hidden rounded-xl border shadow-none">
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody
                  className={
                    isStaffFetching && !isStaffLoading ? 'opacity-60 transition-opacity' : undefined
                  }
                >
                  {isStaffLoading && staffMembers.length === 0 ? (
                    ...
                    staffMembers.map((adm) => (
                      <TableRow key={adm.id} className="group hover:bg-muted/40 transition-colors">
                        <TableCell>
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="text-muted-foreground truncate text-xs">{adm.email}</p>
                            </div>
                          </div>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell className="text-right">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-48">
                              ...
                              <DropdownMenuItem
                                className={`cursor-pointer ${
                                  adm.status === 'Active' ? 'text-destructive' : 'text-emerald-600'
                                }`}
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                onClick={() => setPage((current) => Math.min(staffTotalPage, current + 1))}
              >
                Next
              </Button>
            </div>
        ...
        <div ref={auditSectionRef} className="space-y-4">
          ...
          <div className="bg-card grid grid-cols-2 gap-4 rounded-xl border p-4 sm:grid-cols-4">
            <div className="space-y-1">
              ...
              <select
                ...
                onChange={(e) => {
                  setAuditEntityType(e.target.value || null);
                  setAuditPage(1);
                }}
                className="border-input bg-background h-9 w-full rounded-lg border px-2 text-xs focus:outline-hidden"
              >
            ...
            <div className="space-y-1">
              ...
              >
                ...
                <option value="CLEAR_LOCKOUT">CLEAR_LOCKOUT</option>
              </select>
            </div>
          ...
          <Card className="rounded-xl border shadow-none">
            <Table>
              ...
              <TableBody
                className={
                  isAuditFetching && !isAuditLoading ? 'opacity-60 transition-opacity' : undefined
                }
              >
                {isAuditLoading && auditLogs.length === 0 ? (
                  ...
                  auditLogs.map((log) => (
                    <TableRow key={log.id} className="text-xs">
                      <TableCell>
                        <Badge variant="outline" className="bg-muted/50 text-[10px] font-bold">
                          {log.action}
                        </Badge>
                      </TableCell>
                      ...
                      <TableCell className="text-muted-foreground">
                        {log.actor}
                        <span className="ml-1 text-[10px] opacity-70">({log.actorRole})</span>
                      </TableCell>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                onClick={() => setAuditPage((current) => Math.min(auditTotalPage, current + 1))}
              >
                Next
              </Button>
            </div>
            ...
            <AlertDialogDescription>
              {statusTarget?.status === 'Active' ? (
                <>
                  ...
                  <span className="text-foreground font-semibold">{statusTarget?.name}</span> until
                  reactivated.
                </>
              ) : (
                <>
                  This will restore platform access for{' '}
                  <span className="text-foreground font-semibold">{statusTarget?.name}</span>.
                </>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
      ...
      <Dialog open={isAddAdminOpen} onOpenChange={setIsAddAdminOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Create a new administrative account. System roles define access levels across the
              platform.
            </DialogDescription>
          </DialogHeader>
          ...
          <WkForm onSubmit={onAddAdminSubmit}>
            <div className="space-y-4 py-4">
              <WKInput name="fullName" label="Full Name" placeholder="e.g. Elena Fisher" required />
              <WKInput
                name="email"
                label="Email Address"
                type="email"
              ...
              <div className="grid grid-cols-2 gap-4">
                <WKSelect name="role" label="System Role" options={roleOptions} required />
                <WKInput name="phone" label="Phone (Optional)" placeholder="+1..." />
              </div>
              ...
              <div className="mt-2 rounded-lg border border-amber-200 bg-amber-50 p-3">
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-xs leading-relaxed text-amber-800 italic">
                    Note: A secure magic link will be sent to the {`administrator's`} email for
                    password setup.
                  </p>
                </div>
              </div>
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                <Button type="submit" className="px-8 font-bold" disabled={isCreating}>

...
import type { EmployerAnalyticsPayload, EmployerAnalyticsPeriod } from '@/types/employerAnalytics';
import debounce from 'debounce';
import { Briefcase, CheckCircle2, Crown, FileText, Lock, TrendingUp, Users } from 'lucide-react';
...
export default function AnalyticsView() {
  ...
  const [triggerExportQuery, { isFetching: isExporting }] = useLazyGetEmployerAnalyticsQuery();

  // Job Performance sorting/filtering states (handled on backend)
  const [sortBy, setSortBy] = useState<'views' | 'applications' | 'conversion'>('applications');
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      downloadEmployerAnalyticsCsv(payload, `employer-analytics-${payload.period}.csv`);
    } catch (error) {
        ...
        <Card className="max-w-3xl overflow-hidden border">
          <CardContent className="p-12 text-center">
            ...
            <p className="text-muted-foreground mx-auto mb-10 max-w-lg text-lg leading-relaxed font-medium">
              Gain deep insights into your hiring funnel, job performance, and department
              distribution. Data-driven decisions start with Premium.
            </p>
            ...
            <div className="mb-10 grid grid-cols-1 gap-6 text-left md:grid-cols-2">
              {[
                ...
                <div key={i} className="flex items-start space-x-3">
                  ...
                  <div>
                    <h3 className="text-foreground text-sm font-black">{feature.title}</h3>
                    <p className="text-muted-foreground text-xs font-medium">{feature.desc}</p>
                  </div>
                </div>
              ))}
            </div>
  ...
  return (
    <div className="min-h-screen pt-16">
      ...
      <div className="space-y-6 px-4 pb-10 sm:px-6 sm:py-8">
        {skip && (
          <p className="text-muted-foreground text-sm">Sign in as an employer to view analytics.</p>
        )}
        ...
        {analytics?.hasCompany === false && !isError && (
          ...
          >
            <p>
              Your account is not linked to a company yet. Create or complete your company profile
              to track hiring analytics.
            </p>
        ...
        <AnalyticsOverview summary={analytics?.summary} isLoading={isLoading && !analytics} />

        <Tabs defaultValue="applications" className="space-y-8">
          <div className="text-foreground -mx-4 scrollbar-none overflow-x-auto px-4 sm:mx-0 sm:px-0">

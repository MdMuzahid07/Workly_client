...
const mapStatusToDashboardStatus = (status: string | null | undefined): Job['status'] => {
  switch (status) {
    case 'ACTIVE':
      return 'active';
    case 'DRAFT':
...
const formatPostedDate = (createdAt?: string | Date | null, status?: string | null) => {
...
const DashboardJobManagementView = () => {
  ...
  const handleStatusChange = async (id: string, status: string) => {
    ...
    try {
      ...
      toast.success(`Job ${status === 'ACTIVE' ? 'published' : 'moved to draft'} successfully`);
    } catch (error: any) {
      toast.error(
        error?.data?.errorSources?.message || error?.data?.message || 'Failed to update job status',
      );
    }
  };
  ...
  const confirmCloseAction = async () => {
    ...
    try {
      ...
      toast.error(
        error?.data?.errorSources?.message || error?.data?.message || 'Failed to close job',
      );
    }
    setAlertOpen(false);
    setActionJobId(null);
  ...
  const queryParams = useMemo(() => {
    ...
    return params;
  }, [currentPage, searchQuery, selectedType, selectedExperience, selectedLocation, activeTab]);

  const { data: myJobsData, isLoading: jobsLoading } = useGetMyJobsQuery(queryParams);
  ...
  return (
    <div className="min-h-screen pt-16">
      ...
      <div className="space-y-6 px-4 sm:px-6 sm:py-8">
        ...
        <Dialog open={!!editingJobId} onOpenChange={(open) => !open && setEditingJobId(null)}>
        ...
        <AlertDialog open={alertOpen} onOpenChange={setAlertOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Are you sure you want to close this job posting? It will no longer be visible to
                candidates, but you can still view past applications.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel>Cancel</AlertDialogCancel>
              <AlertDialogAction onClick={confirmCloseAction}>Close Job</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>

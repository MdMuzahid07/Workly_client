...
export default function JobSeekerPricingView() {
  ...
  const isFirstTimeBuyer =
    activePlanName.toLowerCase() === 'free' || !subData?.price || subData.price === 0;
  ...
  const mapDbPlanToCardProps = (plan: any) => {
    ...
    if (discountPercent > 0 && isFirstTimeBuyer) {
      ...
      const discountedValue = Math.floor(basePrice - basePrice * (discountPercent / 100));
  ...
  const starterPlan = plans.find((p: any) => p.name.toLowerCase() === 'starter');
  let starterMinPrice = '৳49';
  if (starterPlan) {
    const planFeatures = typeof starterPlan.features === 'object' ? starterPlan.features : {};
    const discountPercent = Number(planFeatures?.firstTimeDiscountPercent || 0);
        ...
        <div className="px-4 py-8 sm:px-6 lg:px-8">
          <div className="space-y-10">
            ...
            <CandidateSubscriptionStatusCard
              ...
              applicationsLimit={subData?.features?.maxMonthlyApplications || 40}
              ...
              <div className="space-y-6 text-center">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-muted-foreground mx-auto max-w-2xl text-sm sm:text-base">
                    Upgrade with direct HR messaging, enhanced profile visibility, and unlimited
                    applications — starting from just{' '}
                    <strong className="text-foreground">{starterMinPrice}</strong> for your first
                    month.
                  </p>
                </div>
              </div>

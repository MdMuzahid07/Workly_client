...
import { useGetProfileQuery, useGetSavedJobsQuery } from '@/redux/feature/profile/profileApi';
...
export default function JobSeekerDashboardView() {
  ...
  const { data: applicationsData, isLoading: isApplicationsLoading } = useGetMyApplicationsQuery(
    undefined,
    {
      skip: !userId,
    },
  );
  const { data: savedJobsData, isLoading: isSavedLoading } = useGetSavedJobsQuery(undefined, {
    skip: !userId,
  });
  ...
  const savedCount = Array.isArray(savedJobsData?.data) ? savedJobsData.data.length : 0;
                    ...
                    <div className="divide-border divide-y rounded-xl border">
                      <div className="flex items-center justify-between p-2.5 text-xs sm:p-3 sm:text-sm">
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="font-medium">Applications Submitted</span>
                        </div>

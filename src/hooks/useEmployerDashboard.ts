...
import {
  buildApplicationsTrendSubtitle,
  comparePeriodCopy,
...
export function useEmployerDashboard() {
  ...
  const dashboardReady = Boolean(
    isAuthenticated && companyQuery.isSuccess && company?.id,
  );
  ...
  const overview = overviewQuery.data?.data as
    | CompanyOverviewStatistics
    | undefined;
  ...
  const trendCopy = useMemo(() => {
    ...
    const applicationsTrendLine = buildApplicationsTrendSubtitle(
      summary,
      trends,
    );
  ...
  const statsBlocking =
    companyQuery.isLoading || (dashboardReady && overviewQuery.isLoading);

  const detailBlocking =
    dashboardReady && (jobsQuery.isLoading || summaryQuery.isLoading);

  const partialError =
    dashboardReady &&
    (overviewQuery.isError || summaryQuery.isError || jobsQuery.isError);

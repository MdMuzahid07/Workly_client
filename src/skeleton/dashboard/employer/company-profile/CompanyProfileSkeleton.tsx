  ...
  return (
    <div className="bg-background min-h-screen pt-16">
      ...
      <div className="space-y-8 px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="bg-muted/40 border-border flex h-11 w-full scrollbar-none flex-nowrap justify-start gap-1 overflow-x-auto rounded-full border p-1 sm:h-12">
          {['Overview', 'Details', 'Benefits', 'Culture & Values', 'Media'].map((_, i) => (
            <Skeleton key={i} className="h-full w-28 shrink-0 rounded-full sm:w-36" />
          ))}
        </div>
        ...
        <div className="space-y-10">
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
            {Array.from({ length: 2 }).map((_, index) => (
              <Card key={index} className="bg-card space-y-6 rounded-2xl border p-6 shadow-xs">

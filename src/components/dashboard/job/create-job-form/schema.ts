...
export const jobSchema = z
  .object({
    ...
    requirements: z.array(z.string()).min(1, 'At least one requirement is required').default([]),
    ...
    skillsRequired: z
      .array(
        z.object({
          ...
          priority: z.enum(['HIGH', 'MEDIUM', 'LOW', 'GOOD_TO_HAVE']).default('MEDIUM'),
    ...
    // Compensation - Step 3
    salaryMin: z.coerce.number().min(0, 'Minimum salary must be 0 or more').optional().nullable(),
    salaryMax: z.coerce.number().min(0, 'Maximum salary must be 0 or more').optional().nullable(),
    currency: z.string().max(3, 'Currency code must be 3 characters').default('BDT'),

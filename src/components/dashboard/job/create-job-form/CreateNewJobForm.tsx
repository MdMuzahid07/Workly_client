...
export default function CreateNewJobForm({
  ...
  const { data: existingJobData, isLoading: isJobLoading } = useGetJobByIdQuery(jobId as string, {
    skip: !jobId,
  });
  ...
  const handleSubmit = async (data: JobFormData) => {
    try {
      if (!companyId) {
        toast.error('No company linked to your account. Please complete your company profile.');
        return;
      }

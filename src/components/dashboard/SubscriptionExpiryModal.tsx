...
export default function SubscriptionExpiryModal() {
  ...
  useEffect(() => {
    ...
    const isFreePlan =
      planName.toLowerCase() === 'free' || planName.toLowerCase() === 'admin access';
    ...
    if (calculatedDaysLeft >= 0 && calculatedDaysLeft <= 7) {
      ...
      const hasShownThisSession = sessionStorage.getItem('workly_shown_expiry_modal');
          ...
          <div className="flex flex-col items-center space-y-3 text-center">
            ...
            <div className="space-y-1">
              ...
              <DialogDescription className="mt-1 text-xs leading-relaxed text-slate-400">
                Your <span className="font-bold text-amber-400">{subData.planName}</span>{' '}
                subscription is due for manual payment. Renew now to maintain uninterrupted access.
              </DialogDescription>
            </div>
          </div>

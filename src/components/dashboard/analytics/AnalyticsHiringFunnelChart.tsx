...
import type { EmployerConversionMetric, EmployerFunnelStage } from '@/types/employerAnalytics';
import {
  Bar,
  BarChart,
  CartesianGrid,
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {(conversionMetrics.length ? conversionMetrics : []).map((metric) => (
            <div key={metric.label} className="rounded-lg border p-4">
              ...
              <p className="text-primary mt-2 text-3xl font-bold tabular-nums">{metric.value}</p>
            </div>
          ))}
        </div>
      </Card>

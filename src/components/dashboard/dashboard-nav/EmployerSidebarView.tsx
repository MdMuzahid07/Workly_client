...
const EmployerSidebarItem = function EmployerSidebarItem({
  ...
  const isSection =
    normalizedHref !== '/employer' && normalizedPath.startsWith(normalizedHref + '/');
        ...
        >
          ...
          >
            <div className="flex min-w-0 items-center gap-1.5">
              ...
              {(profileData?.data?.isPremium || (user as { isPremium?: boolean })?.isPremium) && (
            ...
            <p className="text-muted-foreground truncate text-xs">Employer Account</p>
          </div>
        </Link>
      </div>
...
export default function EmployerSidebarView() {
  ...
  const navGroups: SidebarGroupProps[] = [
    ...
    {
      title: 'Analytics',
      items: [{ icon: BarChart2, label: 'Analytics', href: '/employer/analytics' }],
    },
  ];

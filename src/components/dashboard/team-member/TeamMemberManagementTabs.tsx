...
interface TeamMemberManagementTabsProps {
  ...
  handleStatusChange: (memberId: string, newStatus: TeamMember['status']) => void;
        ...
        <div className="w-full overflow-x-auto">
          <TabsList className="bg-card grid w-full min-w-[400px] grid-cols-4 sm:min-w-0">
            ...
            <TabsTrigger value="pending" className="text-xs sm:text-sm">
              Pending ({teamMembers.filter((e) => e.status === 'pending').length})
            </TabsTrigger>
            <TabsTrigger value="inactive" className="text-xs sm:text-sm">
              Inactive ({teamMembers.filter((e) => e.status === 'inactive').length})
            </TabsTrigger>
          </TabsList>
        </div>

    ...
    <Card>
      <CardContent className="flex items-center gap-3 p-4">
        ...
        <div className="flex-1 overflow-hidden">
          ...
          <p className={truncate ? 'truncate text-sm font-medium' : 'text-sm font-medium'}>
            {value}
          </p>
        </div>
      </CardContent>
...
const TeamMemberProfileDetails = ({
  ...
  const initials = useMemo(() => getInitials(teamMember.fullName), [teamMember.fullName]);
          ...
          <div className="flex h-full flex-col">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-4">
                  <Avatar className="border-primary/20 h-16 w-16 border-2">
                    <AvatarImage src={teamMember.profile?.avatarUrl || '/placeholder.svg'} />
                  ...
                  <div className="flex-1 space-y-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <SheetTitle className="text-xl">{teamMember.fullName}</SheetTitle>
            ...
            <div className="flex-1 space-y-6 px-6 py-6">
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <IconInfoCard label="Email" value={teamMember.email} Icon={Mail} truncate />

                {teamMember.phone && (
                  <IconInfoCard label="Phone" value={teamMember.phone} Icon={Phone} />
                )}
                ...
                <IconInfoCard label="Join Date" value={teamMember.joinDate} Icon={Calendar} />
              ...
              <div className="flex gap-3">
                <Button className="bg-primary hover:bg-primary/90 flex-1" size="lg">
              ...
              {/* Skills */}
              {teamMember.profile?.skills && teamMember.profile.skills.length > 0 && (
                <>
                  <div className="space-y-3">
                    <h3 className="flex items-center gap-2 font-semibold">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {teamMember.profile.skills.map((skill) => (
                        <Badge key={skill.skillName} variant="secondary" className="px-3 py-1">
              ...
              <Card className="border-success/20 bg-success/5">
                <CardContent className="flex items-center gap-2 p-4">
                  ...
                  <span className="text-sm font-medium">
                    {teamMember.isActive ? 'Active' : 'Inactive'} since {teamMember.joinDate}
                    {teamMember.lastLogin && ` • Last login: ${teamMember.lastLogin}`}
                  </span>
                </CardContent>
              </Card>
            </div>

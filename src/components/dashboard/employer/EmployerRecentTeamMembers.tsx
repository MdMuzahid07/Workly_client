...
import { COMPANY_USER_ROLE_LABELS, type RecentTeamMember } from '@/types/employerDashboard';
...
export function EmployerRecentTeamMembers({ members, isLoading }: EmployerRecentTeamMembersProps) {
  return (
    <Card className="bg-card border">
      <CardHeader>
        <CardTitle className="text-base sm:text-lg">Recent team members</CardTitle>
          ...
          <ul className="space-y-3" aria-label="Recent team members">
            {members.map((member) => (
              ...
              >
                ...
                >
                  {member.createdAt ? format(new Date(member.createdAt), 'M/d/yyyy') : '—'}
                </time>
              </li>
            ))}
          </ul>

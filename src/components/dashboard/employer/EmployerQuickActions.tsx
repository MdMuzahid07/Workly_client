  ...
  return (
    <Card className="bg-card border transition-shadow">
      <CardHeader>
        ...
        <CardDescription className="text-xs sm:text-sm">Shortcuts to common tasks</CardDescription>
      </CardHeader>
      <CardContent>
        <nav
          className="flex scrollbar-none flex-row gap-2.5 overflow-x-auto pb-1 sm:flex-wrap sm:overflow-visible sm:pb-0"
        ...
        >
          {actions.map(({ href, label, icon: Icon, variant }) => (
            <Link key={href} href={href} className="w-auto shrink-0">
              ...
              >
                <Icon className="mr-1.5 h-3.5 w-3.5 shrink-0 sm:mr-2 sm:h-4 sm:w-4" aria-hidden />
                <span>{label}</span>
              </Button>
            </Link>
          ))}

...
const AddSkillDialog = ({
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter" || e.key === ",") {
      ...
      const existingSkillNames = (parentCategory?.taxonomySkills || []).map(
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-3 py-4">
            ...
            <p className="text-muted-foreground text-[10px] leading-relaxed opacity-80">
              * Type a skill name (e.g. React Native, Docker) and press{" "}
              <strong>Enter</strong> or <strong>comma (,)</strong> to add it as
              a tag.
            </p>
          </div>

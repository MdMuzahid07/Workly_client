...
const CustomCategoryHeader = React.forwardRef<HTMLDivElement, EmojiPickerListCategoryHeaderProps>(
...
const CustomRow = React.forwardRef<HTMLDivElement, EmojiPickerListRowProps>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn('grid grid-cols-8 justify-items-center gap-1 py-0.5', className)}
      {...props}
    />
  ),
);
...
const CustomEmoji = React.forwardRef<HTMLButtonElement, EmojiPickerListEmojiProps>(
      ...
      >
        ...
        {open && (
          ...
          >
            ...
            />
            <EmojiPicker.Viewport className="scrollbar-thumb-muted-foreground/10 mt-1 max-h-60 flex-1 [scrollbar-width:none] scrollbar-thin scrollbar-track-transparent overflow-x-hidden overflow-y-auto pr-0.5 [&::-webkit-scrollbar]:hidden">

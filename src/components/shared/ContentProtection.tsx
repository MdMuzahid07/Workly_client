  ...
  useEffect(() => {
    ...
    const handleSecurityEvents = (e: Event) => {
      ...
      if (e.type === 'contextmenu') {
        e.preventDefault();
        showWarning('Right-click menu is disabled to protect platform content.');
        return;
      }
      ...
      if (e.type === 'keydown') {
        ...
        const isMac =
          typeof window !== 'undefined' && /Macintosh|Mac OS X/i.test(navigator.userAgent);

        const isF12 = ke.key === 'F12';
        const isInspect =
          (ke.ctrlKey && ke.shiftKey && (ke.key === 'I' || ke.key === 'i')) ||
          (isMac && ke.metaKey && ke.altKey && (ke.key === 'I' || ke.key === 'i'));
        const isConsole =
          (ke.ctrlKey && ke.shiftKey && (ke.key === 'J' || ke.key === 'j')) ||
          (isMac && ke.metaKey && ke.altKey && (ke.key === 'J' || ke.key === 'j'));
        const isSelectElement =
          (ke.ctrlKey && ke.shiftKey && (ke.key === 'C' || ke.key === 'c')) ||
          (isMac && ke.metaKey && ke.altKey && (ke.key === 'C' || ke.key === 'c'));
        const isViewSource =
          (ke.ctrlKey && (ke.key === 'U' || ke.key === 'u')) ||
          (isMac && ke.metaKey && ke.altKey && (ke.key === 'U' || ke.key === 'u'));
        const isSavePage =
          (ke.ctrlKey && (ke.key === 'S' || ke.key === 's')) ||
          (ke.metaKey && (ke.key === 'S' || ke.key === 's'));

        if (isF12 || isInspect || isConsole || isSelectElement || isViewSource || isSavePage) {
          ke.preventDefault();

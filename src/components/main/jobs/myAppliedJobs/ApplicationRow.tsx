...
export const ApplicationRow = ({
  ...
  const companyHref = app.job.company.slug ? `/companies/${app.job.company.slug}` : undefined;
      ...
      <TableCell className="py-3 pr-3 text-right sm:py-4 sm:pr-6">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56 rounded-xl p-2">
            <DropdownMenuItem asChild className="h-10 cursor-pointer rounded-lg font-medium">
            ...
            {(() => {
              const isWithdrawable =
                (app.status === 'SUBMITTED' || app.status === 'REVIEWING') && !isWithdrawing;
              return (
                <DropdownMenuItem
                  onClick={(e) => {
                    if (!isWithdrawable) {

...
export const SkillsManager = ({
  ...
  const skillsList = useMemo(() => {
    if (selectedCategory === 'all') {
      ...
      const targetCategory = categoriesList.find((cat) => cat.id === selectedCategory);
      const catSkills = (targetCategory?.taxonomySkills || []).map((s) => s.name);
      return catSkills;
    }
  ...
  const getSkillDisplay = (skill: Skill): string => {
    if (skill?.experienceYears === undefined || skill?.experienceYears === null) return 'N/A';
  ...
  const handleAdd = () => {
    if (selectedSkill && experience && onAddTechnicalSkill) {
      const exists = skills.some(
        (s) =>
          (s.skillName || s.skill || '').toLowerCase() === selectedSkill.toLowerCase() &&
          s.type !== 'SOFT',
      );
      if (exists) {
              ...
              <Select value={selectedSkill} onValueChange={setSelectedSkill}>
                ...
                <SelectContent>
                  {skillsList.map((skill) => (
                    <SelectItem className="cursor-pointer" key={skill} value={skill}>
                      {skill}
                    </SelectItem>
                  ))}
                </SelectContent>

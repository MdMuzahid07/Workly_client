...
const CompanyDetailsSidebar = ({ company }: { company: Company }) => {
  ...
  const similarCompaniesList = useMemo(() => {
    const raw = (companiesData?.data?.result ||
      companiesData?.data ||
      []) as Company[];
    ...
    const matching = filtered.filter((c: Company) => {
      const cIndustry =
        typeof c.industry === "string" ? c.industry : c.industry?.name;
      ...
      <Card className="border-primary/10 bg-background/50 border backdrop-blur-sm">
        ...
        <CardContent className="space-y-4">
          {companiesLoading ? (
            ...
            similarCompaniesList.map(
              (similarCompany: Company, index: number) => {
                const openJobsCount =
                  similarCompany._count?.jobs ?? similarCompany.openJobs ?? 0;
                return (
                  <Link
                      ...
                      <div className="mb-2 flex items-center gap-3">
                        {similarCompany.logoUrl ? (
                          ...
                          <div className="bg-primary/10 text-primary flex h-8 w-8 shrink-0 items-center justify-center rounded text-xs font-black">
                            {similarCompany.name
                              ? similarCompany.name[0].toUpperCase()
                              : "C"}
                          </div>
                        )}

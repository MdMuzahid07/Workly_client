...
const WkForm = <T extends FieldValues>({
  ...
  const onFormError = (errors: any) => {
    ...
    if (errorKeys.length > 0) {
      ...
      if (firstError?.message) {
        ...
        const nestedErrorWithMessage = nestedValues.find((v: any) => v?.message) as any;
      ...
      if (message) {
        ...
        toast.error(`Validation error in ${firstErrorKey}. Please check the form.`);
      }
    } else {
  ...
  return (
    <FormProvider {...methods}>
      <form onSubmit={submitHandler(onFormSubmit, onFormError)} className={className}>
        {children}
      </form>
    </FormProvider>
  );

...
const WKIconPicker = ({ name, label, required = false, className }: WKIconPickerProps) => {
  const {
    control,
    formState: { errors },
  } = useFormContext();
  ...
  return (
    <div className={cn('space-y-2', className)}>
              ...
              >
                <Command>
                  ...
                  >
                    ...
                    {Array.from(new Set(IconRegistry.map((i) => i.category))).map((category) => (
                      <CommandGroup key={category} heading={category}>
                        {IconRegistry.filter((i) => i.category === category).map(
      ...
      {hasError && <p className="text-destructive text-sm">{errors[name]?.message as string}</p>}
    </div>
  );
};

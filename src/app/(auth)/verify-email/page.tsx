...
import { setCredentials, updateUser } from '../../../redux/feature/auth/authSlice';
    ...
    const handleVerification = async () => {
      ...
      try {
        ...
        if (data?.accessToken && data?.user) {
          ...
          const redirectUrl =
            data.user.role === 'EMPLOYER' ? '/create-company' : '/dashboard/profile';
        ...
        setStatus('error');
        setErrorMessage(
          error?.data?.message || error?.message || 'Failed to verify email. Please try again.',
        );
      }
    };
      ...
      {status === 'success' && (
        ...
        >
          ...
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground text-sm">
              Your email has been successfully verified! You are now automatically signed in.
              Redirecting you to update your profile...
            </p>
          </div>
          <Button

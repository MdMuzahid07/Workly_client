...
export default function AuthLayout({ children }: { children: React.ReactNode }) {
  ...
  const totalActiveJobs =
    jobsData?.meta?.total || (Array.isArray(jobsData?.data) ? jobsData.data.length : 0);
  ...
  const getPageContent = () => {
    ...
    if (pathname.includes('/verify-email') || pathname.includes('/verification-sent')) {
      return {
        image:
  ...
  const { image, badge, headline, sub, statLabel, statValue } = getPageContent();
      ...
      <aside className="relative hidden h-full flex-col justify-between overflow-hidden bg-slate-950 p-10 text-white select-none lg:flex">
        ...
        <div className="relative z-20 mt-auto space-y-6 pt-20">
          ...
          <div className="max-w-lg space-y-3">
            ...
            <p className="text-base leading-relaxed font-medium text-slate-200/90">{sub}</p>
          </div>

          {/* Real System Metrics & Security Status Bar */}
          <div className="flex items-center gap-6 border-t border-white/15 pt-6">
            <div>
              <p className="text-2xl font-extrabold tracking-tight text-white">{statValue}</p>
              <p className="text-xs font-medium text-slate-300/80">{statLabel}</p>
            </div>

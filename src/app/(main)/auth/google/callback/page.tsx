...
export default function GoogleAuthCallbackPage() {
  ...
  const [confirmGoogleRole, { isLoading: isConfirming }] = useConfirmGoogleRoleMutation();
  ...
  const handleRoleConfirm = async () => {
    ...
    try {
      ...
      toast.error(error?.data?.message || 'Failed to confirm role. Please try again.');
    }
  };
      ...
      <div className="bg-background flex min-h-screen items-center justify-center p-4">
        <div className="w-full max-w-md">
          ...
          <div className="mb-8 text-center">
            ...
            <h1 className="text-foreground text-2xl font-black tracking-tight">Almost there!</h1>
          ...
          <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <p
                  className={`text-sm font-bold ${
                    selectedRole === 'JOB_SEEKER' ? 'text-primary' : 'text-foreground'
                  }`}
                >
                  Job Seeker
                </p>
                <p className="text-muted-foreground mt-0.5 text-xs">Find & apply to jobs</p>
              </div>
              {selectedRole === 'JOB_SEEKER' && <CheckCircle2 className="text-primary h-4 w-4" />}
            </button>
            ...
            >
              ...
              <div>
                ...
                <p className="text-muted-foreground mt-0.5 text-xs">Post jobs & hire talent</p>
              </div>
              {selectedRole === 'EMPLOYER' && <CheckCircle2 className="text-primary h-4 w-4" />}
            </button>
          </div>
  ...
  return (
    <div className="flex min-h-[60vh] items-center justify-center">
      <div className="flex flex-col items-center gap-3">
        ...
        <p className="text-muted-foreground text-sm font-medium">Signing you in with Google…</p>
      </div>
    </div>
  );
}

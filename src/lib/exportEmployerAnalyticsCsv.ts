...
export function buildEmployerAnalyticsCsv(payload: EmployerAnalyticsPayload): string {
  const generatedAt =
  ...
  for (const stage of payload.funnelStages ?? []) {
    lines.push([stage.name, String(stage.count), `${stage.percentage.toFixed(1)}%`]);
  }
  ...
  for (const dept of payload.departments ?? []) {
    lines.push([dept.name, String(dept.count), `${dept.percentage.toFixed(1)}%`]);
  }
